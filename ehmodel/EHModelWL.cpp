// EHModel.cpp -- WangLandau sampling of the Heisenberg model
//             -- Aug 5, 2015 Adjusted to new interfaces/features
//
// Greg Brown (gbrown@fsu.edu,browngrg@comcast.net)
//
// History:
//             
// Sep 26, 2014 -- Adapted from Heisenberg2.cpp
//
...
void ReadLngEst(std::string lng_est_fn, std::vector<double>& energy, std::vector<double>& lng_est)
{
      ...
      if( true )
      {
         ...
         double E,lng;
         std::string buff;
         std::ifstream fin(lng_est_fn.c_str());
...
#  endif
   ...
   using namespace std;
   ProgramOptions options("EHModel","Wang-Landau Sampling.");
   ...
   if( rng_failed )
   {
      cout << "Problem detected with random number generator (returns 1?)" << endl;
   }
   if(verbose) cout << __FILE__ << ":" << __LINE__ << " Random number generator created" << endl;
   if(true) cout << __FILE__ << ": "; wanglandau.urng.print(cout);
