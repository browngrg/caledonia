...
int main(int argc, char* argv[])
{
   ...
   if(false)
   {
      cout << argv[0] << " LngColumn DOSFile1 DOSFile2" << endl;
      cout << "Stitches to DOS Files together. E is assumed first column, Lng column is first argument" << std::endl;
   }
   if(argc<4) return 0;
   ...
   int NGuard = 5;   // How far to stay away from window edge
   int NWin = 5;
   for(int i=0; i<NFILE; i++)
   ...
   fout << "# Column 4: i window left" << endl;
   fout << "# Column 5: ipt left" << endl;
   fout << "# Column 6: Erght" << endl;
   fout << "# Column 7: slope" << endl;
   fout << "# Column 8: lng" << endl;
   fout << "# Column 9: i window right" << endl;
   fout << "# Column 10: i pt right" << endl;
   fout << "# Column 11: slope difference" << endl;
   fout << "# Column 12: sign-corrected slope difference" << endl;
   fout << "# Column 13: used or not" << endl;

   vector<double> Ejoin(NFILE);  Ejoin[0] = window[0].E.front();
   vector<double> shift(NFILE);  shift[0] = 0;
   int ileft_start = NGuard; // 0;
   int iwr = 0;
   int iwl = 0;
   ...
   for(int i=0; i<(NFILE-1); i++)
   {
      ...
      double Elo = window[iwr].E[NGuard];                        // window[iwr].E.front();  // lowest energy of right window
      double Ehi = window[iwl].E[window[iwl].E.size()-1-NGuard];   // window[iwl].E.back();   // highest energy of left window
      if( Elo>Ehi )
      {
         cout << "Windows do not overlap i=" << i << endl;
         return 1;
      }
      cout << "i=" << i << " Elo=" << Elo << " Ehi=" << Ehi << " NGuard=" << NGuard << endl;
      ...
      for(int ipt=ileft_start; ipt<ileft; ipt++)
      {
         ...
         fout << window[iwl].E[ipt] << " " << window[iwl].slope[ipt] << " " << window[iwl].lng[ipt] << " " << iwl << " " << ipt << " "
              << window[iwr].E[irght] << " " << window[iwr].slope[irght] << " " << window[iwr].lng[irght] << " " << iwr << " " << irght << " "
              << diff << " " << foo*diff << " " << 2 << endl;
      }
      irght   = window[iwr].find_energy(window[iwl].E[ileft]);
      diff = window[iwr].slope[irght]- window[iwl].slope[ileft];  
      foo = (diff>0)? 1 : -1;  // 2*(diff>0) - 1;                                        // -1 or 1
      while( (foo*diff)>0 && ileft<(window[iwl].E.size()-NGuard) )
      {
         merge_E.push_back( window[iwl].E[ileft] );
         merge_lng.push_back( window[iwl].lng[ileft] + shift[i] );
         fout << window[iwl].E[ileft] << " " << window[iwl].slope[ileft] << " " << window[iwl].lng[ileft] << " " << iwl << " " << ileft << " "
              << window[iwr].E[irght] << " " << window[iwr].slope[irght] << " " << window[iwr].lng[irght] << " " << iwr << " " << irght << " "
              << diff << " " << foo*diff << " " << 1 << endl;
         ileft++;
         irght = window[iwr].find_energy( window[iwl].E[ileft] );
         diff = window[iwr].slope[irght]- window[iwl].slope[ileft];  
      }
   ...
   double lng_max = *(std::max_element(merge_lng.begin(),merge_lng.end()));
   for(int ipt=0; ipt<merge_lng.size(); ipt++) merge_lng[ipt] -= lng_max;
