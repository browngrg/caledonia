...
#  endif
   ...
   // Local options
   int ThetaBlo = 2;
   int ThetaBhi= (64*64/4);
   options.add_option( "ThetaBlo", "Lowest value of ThetaB for run",  ' ', &(ThetaBlo));
   options.add_option( "ThetaBhi", "Highest value of ThetaB for run", ' ', &(ThetaBhi));
...
#  endif
   ...
   char fname[100];
   sprintf(fname,"MfiaWLfixedHist-%d.csv",ithread);
   ...
   if(ThetaBlo<2) ThetaBlo=2;
   if(ThetaBhi<(NGrid/4)) ThetaBhi=NGrid/4;
   for(int ThetaB=ThetaBlo; ThetaB<=ThetaBhi; ThetaB+=1)
   {
      for(int ThetaA=ThetaB+ithread; ThetaA<=(NGrid/2-ThetaB); ThetaA+=nthread)
      {
